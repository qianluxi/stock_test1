...
class IndicatorEngine:
    ...
    def add_atr(self, df):
        ...
        atr = df.groupby("symbol")["tr"].transform(lambda x: x.rolling(14).mean())

        # ⭐ 标准化（关键）
        df["atr14"] = atr / df["close_adj"]
    ...
    def add_momentum(self, df):
        ...
        df["momentum10"] = df.groupby("symbol")["close_adj"].pct_change(10)
    ...
    def add_cci(self, df):
        ...
        md = tp.groupby(df["symbol"]).transform(
            lambda x: x.rolling(20).apply(lambda y: np.mean(np.abs(y - y.mean())))
        )
